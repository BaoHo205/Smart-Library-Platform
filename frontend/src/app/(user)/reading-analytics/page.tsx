...
export default function ReadingAnalyticsPage() {
    ...
    const [highlightedBooks, setHighlightedBooks] = useState<MostHighlightedBook[]>([]);
        ...
        if (data.length === 0) {
            ...
            for (let d = new Date(startDate); d <= endDate; d.setMonth(d.getMonth() + 1)) {
                ...
                filledData.push({
                    ...
                    fullMonthLabel: d.toLocaleDateString('en-US', { month: 'short', year: 'numeric' }),
                    totalSessions: 0,
                    totalDuration: 0,
                    avgDuration: 0,
                    uniqueBooksCount: 0,
    ...
    useEffect(() => {
        ...
        const loadInitialData = async () => {
            try {
                ...
                const filledChartData = fillMissingMonths(chartTrendsData, filters.months, filters.dateRange);

                if (filledChartData.length > 0) {
                    const totalDuration = filledChartData.reduce((sum, trend) => sum + trend.totalDuration, 0);
    ...
    const handleFiltersChange = useCallback((newFilters: Partial<AnalyticsFiltersState>) => {
