...
export function AnalyticsFilters({
  ...
  const [localHighlightedLimit, setLocalHighlightedLimit] = useState(
    filters.highlightedBooksLimit.toString()
  );
  const [localTopBooksLimit, setLocalTopBooksLimit] = useState(
    filters.topBooksLimit.toString()
  );

  const { users, loading: usersLoading } = useUserSearch(
    viewMode === 'platform' ? userSearchInput : ''
  );
  ...
  const handleQuickLimitChange = (
    type: 'highlighted' | 'topBooks',
    value: string
  ) => {
    if (value === 'max') {
      const limit = 9999;
      onFiltersChange({
        [type === 'highlighted' ? 'highlightedBooksLimit' : 'topBooksLimit']:
          limit,
      });
            ...
            <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-4">
              ...
              <div className="space-y-3">
                ...
                <p className="text-xs text-gray-500">
                  {filters.months === 'all'
                    ? 'Reading data from first to last available'
                    : `Reading data from the last ${filters.months || 6} months`}
                </p>
              </div>
              ...
              <div className="space-y-3">
                ...
                <Select
                  ...
                  onValueChange={value =>
                    handleQuickLimitChange('highlighted', value)
                  }
                >
                  <SelectTrigger className="border-gray-200 bg-white transition-colors hover:border-gray-300">
                    <SelectValue>
                      {localHighlightedLimit === 'max'
                        ? 'Max (See all)'
                        : `Top ${localHighlightedLimit} Books`}
                    </SelectValue>
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div className="space-y-3">
                ...
                <Select
                  ...
                  onValueChange={value =>
                    handleQuickLimitChange('topBooks', value)
                  }
                >
                  <SelectTrigger className="border-gray-200 bg-white transition-colors hover:border-gray-300">
                    <SelectValue>
                      {localTopBooksLimit === 'max'
                        ? 'Max (See all)'
                        : `Top ${localTopBooksLimit} Books`}
                    </SelectValue>
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="5">Top 5 Books</SelectItem>
            ...
            <CardContent className="border-t border-gray-100 bg-gray-50 pt-6 pb-8">
              <div className="space-y-6">
                ...
                <div className="flex flex-col gap-4 lg:flex-row lg:gap-6">
                  ...
                  {viewMode === 'platform' && (
                    <Card className="flex-1 border-2 border-purple-100 bg-gradient-to-br from-purple-50 to-white">
                      <CardContent className="p-4">
                        <div className="mb-4 flex items-center gap-3">
                      ...
                      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div>
                          ...
                          <Input
                            ...
                            value={
                              localHighlightedLimit === 'max'
                                ? ''
                                : localHighlightedLimit
                            }
                            onChange={e =>
                              setLocalHighlightedLimit(e.target.value)
                            }
                            onBlur={() =>
                              handleHighlightedLimitChange(
                                localHighlightedLimit
                              )
                            }
                            onKeyDown={e => {
                        ...
                        <div>
                          ...
                          <Input
                            ...
                            onChange={e =>
                              setLocalTopBooksLimit(e.target.value)
                            }
                            onBlur={() =>
            ...
            <CardContent className="border-t border-gray-100 pt-4 pb-6">
              <div className="flex flex-wrap items-center gap-3">
                ...
                {filters.highlightedBooksLimit !== 5 && (
                  ...
                  >
                    Top{' '}
                    {filters.highlightedBooksLimit === 9999
                      ? 'Max'
                      : filters.highlightedBooksLimit}{' '}
                    highlighted
                  </Badge>
                )}
                {filters.topBooksLimit !== 10 && (
                  <Badge
                    variant="outline"
                    className="border-orange-200 bg-orange-50 text-orange-700 transition-colors hover:bg-blue-100"
                  >
                    Top{' '}
                    {filters.topBooksLimit === 9999
                      ? 'Max'
                      : filters.topBooksLimit}{' '}
                    by time
                  </Badge>
                )}
              </div>
