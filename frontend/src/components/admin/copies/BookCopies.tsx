...
const BookCopies: React.FC<BookCopiesProps> = ({ id }) => {
    ...
    const handleDeleteCopy = async (copyId: string) => {
        ...
        try {
            ...
            const errorMessage = (error as any).response?.data?.message || 'Failed to delete book copy.';
    ...
    const handleAddCopy = async () => {
        setIsAdding(true);
        try {
            const response = await axiosInstance.post(`/api/v1/books/copy/create/${id}`);
            ...
            const errorMessage = (error as any).response?.data?.message || 'Failed to add a new book copy.';
            toast.error(errorMessage);
        } finally {
            setIsAdding(false);
        }
